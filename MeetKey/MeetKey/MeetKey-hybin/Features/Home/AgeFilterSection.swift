...
// MARK: - Preview
struct AgeFilterSection_Preview: View {
    ...
    var body: some View {
        VStack(spacing: 30) {
            ...
            AgeFilterSection(
                minAge: $filterModel.minAge,
                maxAge: $filterModel.maxAge
            )
            ...
            VStack(alignment: .leading, spacing: 8) {
                Text("현재 상태:")
                    .font(.caption)
                    .foregroundColor(.secondary)
                Text("minAge: \(filterModel.minAge.map { "\($0)" } ?? "nil")")
                    .font(.caption)
                    .monospaced()
                Text("maxAge: \(filterModel.maxAge.map { "\($0)" } ?? "nil")")
                    .font(.caption)
                    .monospaced()
            }
            .padding()
