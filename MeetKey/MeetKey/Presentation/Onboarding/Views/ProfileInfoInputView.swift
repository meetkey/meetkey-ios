            ...
            VStack(spacing: 0) {
                        ...
                        TextField("", text: $viewModel.data.name)
                            ...
                            .background(
                                RoundedRectangle(cornerRadius: 12)
                                    .stroke(Color.meetKeyBlack04, lineWidth: 1)
                            )
                            ...
                            ForEach(genders, id: \.self) { gender in
                                ...
                                Button(action: {
                                    ...
                                    Text(gender)
                                        ...
                                        .foregroundColor(isSelected ? .meetKeyOrange04 : .meetKeyBlack06)
                                        .frame(maxWidth: .infinity)
                                        .frame(height: 56)
                                        .background(
                                            RoundedRectangle(cornerRadius: 12)
                                                .fill(isSelected ? Color.meetKeyOrange05 : Color.white)
                                        )
                                        .overlay(
                                            RoundedRectangle(cornerRadius: 12)
                                                .strokeBorder(isSelected ? Color.meetKeyOrange04 : Color.meetKeyBlack04, lineWidth: isSelected ? 1.5 : 1)
                                        )
                                }
                            }
                        }
                ...
                NavigationLink(destination: ProfileImageUploadView(viewModel: viewModel)) {
                    Text("다음")
                        ...
                        .frame(maxWidth: .infinity)
                        .frame(height: 54)
                        .background(viewModel.isProfileInfoCompleted ? Color.meetKeyOrange04 : Color.meetKeyBlack04)
                        .cornerRadius(15)
                }
...
struct LabelWithStar: View {
    ...
    var body: some View {
        HStack(spacing: 2) {
            Text(text).font(.custom("Pretendard-SemiBold", size: 16)).foregroundColor(.meetKeyBlack01)
            Text("*").font(.custom("Pretendard-SemiBold", size: 16)).foregroundColor(.meetKeyOrange04)
        }
    }
}
